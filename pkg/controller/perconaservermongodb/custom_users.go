...
func handleUsers(ctx context.Context, cr *api.PerconaServerMongoDB, mongoCli mongo.Client, client client.Client, annotationKeySuffix string) error {
	...
	for _, user := range cr.Spec.Users {
		...
		// LIB-876: annotation key is shard-aware so custom-user propagation to
		// shard replsets stays idempotent. When no suffix is supplied (non-sharded
		// / cluster-wide path) fall back to the base key.
	...
	// LIB-876: connection-string secrets are a cluster-level concern and are only
	// built on the cluster-wide pass (empty suffix). The per-shard passes
	// (suffix == replset name) reuse the same client/user set and must not
	// re-derive connection strings, which would duplicate work and require
	// mongos/TLS config that is irrelevant to the shard-local user propagation.
	if annotationKeySuffix == "" {
