...
func TestScaleDownStatefulSetsForSnapshotRestore(t *testing.T) {
  ...
  enableEncryption: false`),
	...
	t.Run("encryption not specified defaults to disabled (LIB-982) uses plain pbm-agent command", func(t *testing.T) {
		...
		container := updated.Spec.Template.Spec.Containers[0]
		// LIB-982: community MongoDB images default encryption to DISABLED, so an
		// unspecified encryption config behaves like explicitly-disabled: the plain
		// pbm-agent command is used with no db-config wrapper or volume.
		assert.Equal(t, []string{"/opt/percona/pbm-agent"}, container.Command)
		assert.NotContains(t, container.Args, "--db-config")
		...
		assert.False(t, foundVolume, "expected no pbm-db-config volume when encryption defaults to disabled (LIB-982)")
	})
}
...
func TestCreateOrUpdateDBConfigSecret(t *testing.T) {
	...
	t.Run("no secret created when encryption is not configured because it defaults to disabled (LIB-982)", func(t *testing.T) {
		cluster := makeCluster("")
		r := fakeReconciler(cluster)
		...
		// LIB-982: community MongoDB images default encryption to DISABLED, so an
		// unspecified encryption config does not produce a pbm db-config secret.
		secret := &corev1.Secret{}
		err = r.client.Get(t.Context(), types.NamespacedName{Name: expectedSecretName, Namespace: ns}, secret)
		assert.True(t, k8sErrors.IsNotFound(err), "expected no db-config secret when encryption defaults to disabled")
	})
